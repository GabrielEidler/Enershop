jQuery(document).ready(function () {
    ...
    const residence_Recommendation_Calculator = (userObject) =>{
      ...
      if(userObject.city === 'Florianópolis, SC, Brasil'){
        //gd não disponível
        if (userObject.equationScore < 9.8) {
          if (userObject.type === 'apartamento') {
            $('.residence_pr').removeClass('hidden');
          }else{
            $('.residence_s-pr').removeClass('hidden');
          }
        }else {
          if (userObject.type === 'apartamento') {
            $('.residence_pr').removeClass('hidden');
    ...
    const business_Recommendation_Calculator = (userObject) =>{
      ...
      if (userObject.equationScore < 9.8) {
        if (userObject.voltage === 'a') {
          $('.residence_s-gdd-pc').removeClass('hidden');
        }else if (userObject.voltage === 'b'){

          $('.residence_gdc-s-pc').removeClass('hidden');
        ...
        if (userObject.voltage === 'a') {
          ...
          $('.residence_s-gdc-pc').removeClass('hidden');
    ...
    const quizzSystem = () => {
      ...
      const businessQuestions = () => {
        ...
        $('.business-address_plus-button').click(()=>{
          if($('.business-address_tag').hasClass('hidden')) $('.business-address_tag').removeClass('hidden');
